...
- **Type:** unauthorized access to a fund-controlling function (price authority) → economic drain. This qualifies on the access-control bar irrespective of amount; it also clears the economic bar overwhelmingly (profit ≈ pool size, independent of attacker stake).
- **Cost to execute (measured, not the take):** upfront own capital **$0** (the attacker EOA sent 0 USDC in, all 3 txs); flash-loan principal (10,000 USDC ×3) is borrowed **and repaid same tx** so it is not a cost — only the **15 USDC** total fee is; gas across all 3 txs was **0.000007 ETH (~$0.01)** (Base sub-gwei). Capital at risk on failure is **$0** (atomic). So real cost ≈ **$15**, net profit **3.32M USDC** → **~220,000× return**. This is the "gas + flash-loan, not $6M of own capital" case: there is **no capital barrier and nothing at stake** — the exploit is self-financing (the ~$338 position margin is carved from the flash loan; the fee is paid from proceeds within the same tx).
- **The flash fee is confirmed by execution, not just by reading history:** I deployed a minimal flash-borrower (`audit/poc/FlashTest.sol`) on the fork and actually borrowed 10,000 USDC from the very pool the attacker used (UniV3 WETH/USDC, fee tier **500 = 0.05%**); the fee read live inside `uniswapV3FlashCallback` was **exactly 5.000000 USDC** (and the pool's balance rose by exactly 5). So $5/tx → **$15** total is confirmed three ways: fee-tier arithmetic, the attacker's real repayment (10,000→10,005), and this live execution. (The *full* atomic attack replay reverts downstream at the router on fork state — the flash loan executes and `0xd649` receives the USDC; the revert is a position-leg reconstruction artifact, not flash-related. Details: `audit/poc/flashloan_execution.md`.) Full ledger: `audit/poc/RESULTS.md`.
